...
Опять же энергопотребление находится в прямой зависимости от разрешения симуляции (level of details).
...
# Абстрактная модель человеческого тела
Не имеет смысл хранить тело человека клета за клеткой, в представлении данных тела может использоваться механизм абстракции данных.
Проект OpenWorm представляет собой чрезвычайно подробную компьютерную модель нематоды Caenorhabditis elegans. Модель симулирует все аспекты организма, включая его анатомию, физиологию, поведение и нейронную активность. Несмотря на свою сложность, модель OpenWorm по-прежнему является неполной. Например, модель не симулирует пока процессы роста и старения. 
В симуляции старение может быть заменено чисто визуальными признаками, а старение отдельных клеток будет симулироваться только если в будущих сценариях игрок столкнется с каким-то серьезными отказами органов, решит проанализировать свои клетки в микроскоп. Т.е. есть некий асбстракная модель клетки homo-sapience для разных возрастов. Эти модели не учитывают особенности конкретной ДНК, но реагируют на внешнюю среду так как все организмы реагируют.
Возможно в некоторых случиях симулируется только старение кожи.
